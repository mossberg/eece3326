...
int board::findNextBlank(int &i, int &j)
// returns 1 if found blank
// returns 0 if didn't find blank
{
    for (int i_ = 1; i_ <= BoardSize; i_++)
    {
        for (int j_ = 1; j_ <= BoardSize; j_++)
        {
            if (isBlank(i_, j_))
            {
                i = i_;
...
bool board::solve(int i, int j) // int i, j are coordinates to look in
{
    ...
    system("clear");
    print();
    //cout << "Currently solving at: " << i << " " << j << " " << recCounter << endl;
    ...
    if (isSolved()) {
        ...
        for (int val = MinValue; val <= MaxValue; val++) {
            //cout << "Trying value " << val << endl;
            if (hasConflicts(i, j, val)) {
                //cout << "There was a conflict! continuing next val " << endl;
                continue;
            } else {
                //cout << "there was no conflict!" << endl;
                // legal value to place in cell
                value[i][j] = val;
                int next_i = 0;
                int next_j = 0;
...
int main()
{
    ...
    try
    {
        ...
        while (fin && fin.peek() != 'Z')
        {
            ...
            int first_i = 0;
            int first_j = 0;
            b1.findNextBlank(first_i, first_j);
            
            cout << first_i << " " << first_j << endl;
            // b1.solve(first_i, first_j);
